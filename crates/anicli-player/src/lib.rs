...
pub fn launch(request: &PlaybackRequest) -> Result<PlaybackOutcome> {
    if request.log_episode && !matches!(request.player, PlayerKind::Debug | PlayerKind::Download) {
...
fn launch_iina(program: &str, request: &PlaybackRequest) -> Result<PlaybackOutcome> {
...
fn launch_mpv(program: &str, request: &PlaybackRequest) -> Result<PlaybackOutcome> {
    if program == "flatpak_mpv" {
        let mut args = vec!["run".to_owned(), "io.mpv.Mpv".to_owned()];
        args.extend(mpv_args(request));
        return spawn_or_wait("flatpak", args, !request.no_detach, request.exit_after_play);
    }
    spawn_or_wait(
        program,
        mpv_args(request),
...
fn mpv_args(request: &PlaybackRequest) -> Vec<String> {
    let mut args = vec![format!("--force-media-title={}", request.media_title())];
    if let Some(subtitle) = &request.stream.subtitle {
...
fn launch_vlc(program: &str, request: &PlaybackRequest) -> Result<PlaybackOutcome> {
...
fn launch_syncplay(program: &str, request: &PlaybackRequest) -> Result<PlaybackOutcome> {
...
fn launch_custom(program: &str, request: &PlaybackRequest) -> Result<PlaybackOutcome> {
    spawn_or_wait(program, vec![request.stream.url.clone()], true, false)
}

fn download(request: &PlaybackRequest) -> Result<PlaybackOutcome> {
    std::fs::create_dir_all(&request.download_dir)
        .wrap_err_with(|| format!("failed to create {}", request.download_dir.display()))?;
    if let Some(subtitle) = &request.stream.subtitle {
