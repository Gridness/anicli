...
impl AniSkipClient {
	pub fn new() -> Result<Self> {
		...
		headers.insert(
			header::USER_AGENT,
			header::HeaderValue::from_static(AGENT),
		);
		headers.insert(
			header::CONTENT_TYPE,
	...
	pub async fn resolve_mal_id(
		...
		match source {
			SkipSource::MyAnimeList => {
				self.fetch_mal_id_myanimelist(query_or_id, filter).await
			}
			SkipSource::AllAnime => {
				self.fetch_mal_id_allanime(query_or_id, filter).await
			}
		}
	}

	pub async fn resolve_mal_id_from_allanime_id(
		&self,
		allanime_id: &str,
	) -> Result<u64> {
		let value = self
	...
	pub async fn skip_times(
		...
		let url = format!(
			"https://api.aniskip.com/v1/skip-times/{mal_id}/{episode}?types=op&types=ed"
		);
		let response = self
			.http
			.get(&url)
	...
	async fn fetch_mal_id_myanimelist(
		&self,
		query: &str,
		filter: Option<&str>,
	) -> Result<u64> {
	...
	async fn fetch_mal_id_allanime(
		&self,
		query: &str,
		filter: Option<&str>,
	) -> Result<u64> {
		...
		let selected = edges
			...
			.find(|edge| {
				let title =
					edge.get("name").and_then(Value::as_str).unwrap_or("");
				filter_re
					.as_ref()
		...
		let id =
			selected.get("_id").and_then(Value::as_str).ok_or_else(|| {
				eyre!("selected AllAnime result did not include an id")
			})?;
...
fn select_mal_item(
	...
	selected.get("id").and_then(Value::as_u64).ok_or_else(|| {
		eyre!("selected MyAnimeList result did not include an id")
	})
}
