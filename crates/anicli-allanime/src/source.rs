...
impl AllAnimeClient {
    ...
    pub async fn search(
        ...
        let edges = value
            ...
            .ok_or_else(|| eyre!("AllAnime search response did not include show edges"))?;
        ...
        Ok(edges
            ...
            .filter_map(|edge| {
                ...
                let episode_count = edge
                    .pointer(&format!("/availableEpisodes/{}", mode.as_allanime()))
    ...
    pub async fn episodes(&self, show_id: &str, mode: TranslationMode) -> Result<Vec<String>> {
        let query_text =
        ...
        let episodes = value
            ...
            .and_then(Value::as_array)
            .ok_or_else(|| eyre!("AllAnime response did not include episode details"))?;
    ...
    pub async fn episode_sources(
        ...
        let selected = select_quality(&links, quality)
            .ok_or_else(|| eyre!("episode is released, but no valid sources were found"))?;
    ...
    async fn source_refs(
        ...
        let value: Value = serde_json::from_str(&response).wrap_err("invalid AllAnime JSON")?;
    ...
    async fn fetch_provider_links(&self, source_ref: &SourceRef) -> Result<Vec<StreamLink>> {
        ...
        let response = self
            ...
            .send()
            .await
            .wrap_err_with(|| format!("failed to fetch provider endpoint {url}"))?
            .error_for_status()
            .wrap_err_with(|| format!("provider endpoint returned an error for {url}"))?
            .text()
            .await
    ...
    async fn post_graphql_text_with_referer(&self, body: Value, referer: &str) -> Result<String> {
        self.http
...
pub fn select_quality(links: &[StreamLink], quality: &QualityPreference) -> Option<SelectedStream> {
    match quality {
...
fn decode_tobeparsed_from_response(response: &str) -> Result<Vec<SourceRef>> {
    ...
    let plain = decode_tobeparsed_gcm(&bytes).or_else(|_| decode_tobeparsed_ctr(&bytes))?;
...
fn decode_tobeparsed_gcm(bytes: &[u8]) -> Result<String> {
    ...
    let cipher = Aes256Gcm::new_from_slice(&key).wrap_err("failed to initialize AES-GCM")?;
    let nonce = Nonce::from_slice(&bytes[1..13]);
    let decrypted = cipher
        .decrypt(nonce, &bytes[13..])
        .map_err(|_| eyre!("failed to decrypt AllAnime source blob as AES-GCM"))?;
    String::from_utf8(decrypted).wrap_err("decrypted AllAnime source blob is not UTF-8")
}
...
fn decode_tobeparsed_ctr(bytes: &[u8]) -> Result<String> {
    ...
    String::from_utf8(ciphertext).wrap_err("decrypted AllAnime source blob is not UTF-8")
}
...
pub fn decode_provider_path(encoded: &str) -> Result<String> {
    ...
    for pair in encoded.as_bytes().chunks(2) {
        ...
        let hex = std::str::from_utf8(pair).wrap_err("provider path is not valid UTF-8")?;
        let byte = u8::from_str_radix(hex, 16).wrap_err("provider path is not hex encoded")?;
...
fn source_refs_from_value(value: &Value) -> Vec<SourceRef> {
    match value {
        Value::Array(values) => values.iter().flat_map(source_refs_from_value).collect(),
        Value::Object(map) => {
...
fn source_refs_from_text(value: &str) -> Vec<SourceRef> {
    let normalized = value
        ...
        .replace("\\\"", "\"");
    let forward_re =
        Regex::new(r#""sourceUrl":"([^"]+)".*?"sourceName":"([^"]+)""#).expect("valid regex");
    let reverse_re =
        Regex::new(r#""sourceName":"([^"]+)".*?"sourceUrl":"([^"]+)""#).expect("valid regex");
    let mut refs = Vec::new();

    for captures in forward_re.captures_iter(&normalized) {
        if let Some(source_ref) = source_ref_from_fields(&captures[2], &captures[1]) {
            refs.push(source_ref);
        }
    }
    for captures in reverse_re.captures_iter(&normalized) {
        if let Some(source_ref) = source_ref_from_fields(&captures[1], &captures[2]) {
            refs.push(source_ref);
        }
    }
