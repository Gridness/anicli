...
pub fn next_episode<'a>(episodes: &'a [String], current: &str) -> Option<&'a str> {
    episodes
        .iter()
...
pub fn previous_episode<'a>(episodes: &'a [String], current: &str) -> Option<&'a str> {
    episodes
        .iter()
        .position(|episode| episode == current)
...
pub fn parse_episode_range(selection: &str, episodes: &[String]) -> Result<Vec<String>> {
