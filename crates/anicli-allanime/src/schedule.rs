...
pub async fn fetch_next_episode_status(query: &str) -> Result<Vec<NextEpisodeStatus>> {
    let client = Client::new();
    let search_url = format!("https://animeschedule.net/api/v3/anime?q={}", encode(query));
    let search = client
        .get(&search_url)
        .send()
        .await
